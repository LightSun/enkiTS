...
namespace gn
{
	...
	{
		GN_TRACE(kTraceApiCall, kTraceClientKernel,0, "SetTraceFilter(filter=0x%x)", filter);
		...
		{
			GN_TRACE(kTraceApiCall, kTraceClientKernel, -1, "SetTraceFilter(filter=0x%x)", filter);
		}
	...
	int GnLog::setTraceFile(const char* fileNameUTF8, bool addFileCounter)
	{
		...
		GN_TRACE(kTraceApiCall, kTraceClientKernel,0, "SetTraceFile(fileNameUTF8=%s, addFileCounter=%d)", fileNameUTF8, addFileCounter);
		return (ret);
	}

	int GnLog::setTraceCallback(TraceCallback* callback)
	{
		GN_TRACE(kTraceApiCall, kTraceClientKernel,0, "SetTraceCallback(callback=0x%x)", callback);
